...
配置中的设备名称字符串会由 `AlsaBackend` **直接传递**给 ALSA 的 `PCM::new()` 接口（见 `src/audio/alsa_device.rs`），程序本身不做任何转换或解析，填写时需确保设备名称为合法的 ALSA PCM 设备名。音频系统通过 `AudioBackend` 与 `AudioFrontend` 接口解耦硬件 I/O 和前处理，测试可使用 `MockBackend` 与 `NullFrontend`，不需要真实声卡。

...
7. **受控子进程**：Subprocess 工具由统一 supervisor 管理进程组；超时、取消或程序退出时会终止子进程及其后代，stdout/stderr 分别限制为 1 MiB。
...
适用于**执行时间长**的工具。网关立刻向大模型返回 `{"status": "started"}`，后台任务由 `ProcessSupervisor` 跟踪。大模型会告知用户"任务已启动"，对话可以继续；程序退出时网关会取消并回收仍在运行的任务。
...
- **生命周期**：每次调用创建独立进程组，超时或关闭时终止整个进程树；stdout 和 stderr 各自最多保留 1 MiB。

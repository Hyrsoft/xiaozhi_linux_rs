...
# =============================================================================
# riscv64gc-unknown-linux-gnu 混合链接编译脚本
#
# 本脚本会自动完成以下步骤：
...
echo "============================================="
echo "  混合链接编译 - riscv64gc-unknown-linux-gnu"
echo "============================================="
...
#   CROSS_COMPILER_PREFIX - 自定义编译器前缀（默认: riscv64-unknown-linux-gnu）
...
if [ -n "$CROSS_TOOLCHAIN_DIR" ]; then
    ...
    # === 默认逻辑：自动下载工具链 ===
    TOOLCHAIN_NAME="riscv64-wangzai-linux-gnu-gcc"
    TOOLCHAIN_URL="https://github.com/Hyrsoft/xiaozhi_linux_rs/releases/download/Source_Mirror/riscv64-unknown-linux-gnu-gcc.tar.gz"
...
export CC_riscv64gc_unknown_linux_gnu="$CROSS_GCC"
export CXX_riscv64gc_unknown_linux_gnu="$CROSS_CXX"
export AR_riscv64gc_unknown_linux_gnu="$CROSS_AR"

# Cargo linker
export CARGO_TARGET_RISCV64GC_UNKNOWN_LINUX_GNU_LINKER="$CROSS_GCC"

# 告诉 Rust cc crate 编译 C 源码时使用 -fPIC（PIE 二进制必需）
export CFLAGS_riscv64gc_unknown_linux_gnu="-fPIC${EXTRA_CFLAGS:+ $EXTRA_CFLAGS}"
...
# --no-warn-mismatch：GCC 8 的 ld 不识别 Rust compiler_builtins 中的新 RISC-V Z 扩展属性，
#   需要跳过 ISA 属性合并错误（不影响运行时正确性）
export RUSTFLAGS="-C link-arg=-L$ALSA_SHARED_LIBDIR -C link-arg=-Wl,--no-as-needed -C link-arg=-ldl -C link-arg=-lpthread -C link-arg=-lm -C link-arg=-Wl,--no-warn-mismatch${EXTRA_RUSTFLAGS:+ $EXTRA_RUSTFLAGS}"
